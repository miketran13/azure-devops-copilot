...
> **Required Azure role:** The identity running the deployment (your CLI account or the GitHub Actions service principal) must have **Owner** or **User Access Administrator** on the resource group. `Contributor` alone is not sufficient — the Bicep templates create RBAC role assignments for the Function App's managed identity to access storage and Key Vault.
...
4. Grant the service principal **Owner** on the resource group (required for RBAC role assignments in Bicep):
    ...
    > Alternatively, grant `User Access Administrator` + `Contributor` if your policy restricts the `Owner` role.

5. Push to `main` — the `deploy-backend.yml` workflow will deploy automatically.
